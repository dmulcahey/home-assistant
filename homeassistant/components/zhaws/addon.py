...
from .const import ADDON_SLUG, CONF_ADDON_DEVICE, DOMAIN, LOGGER
...
def api_error(
    error_message: str,
) -> Callable[[Callable[_P, Awaitable[_R]]], Callable[_P, Coroutine[Any, Any, _R]]]:  # type: ignore
    """Handle HassioAPIError and raise a specific AddonError."""

    def handle_hassio_api_error(
        func: Callable[_P, Awaitable[_R]]  # type: ignore
    ) -> Callable[_P, Coroutine[Any, Any, _R]]:  # type: ignore
        """Handle a HassioAPIError."""

        async def wrapper(*args: _P.args, **kwargs: _P.kwargs) -> _R:  # type: ignore
...
class AddonManager:
    ...
    @api_error("Failed to get ZHAWS add-on discovery info")
    async def async_get_addon_discovery_info(self) -> dict:
        ...
        if not discovery_info:
            raise AddonError("Failed to get ZHAWS add-on discovery info")

        discovery_info_config: dict = discovery_info["config"]
        return discovery_info_config

    @api_error("Failed to get the ZHAWS add-on info")
    ...
    def async_get_addon_state(self, addon_info: dict[str, Any]) -> AddonState:
        """Return the current state of the ZHAWS add-on."""
    ...
    def async_schedule_install_setup_addon(
        ...
        """Schedule a task that installs and sets up the ZHAWS add-on.
    ...
    @api_error("Failed to uninstall the ZHAWS add-on")
    async def async_uninstall_addon(self) -> None:
        """Uninstall the ZHAWS add-on."""
        await async_uninstall_addon(self._hass, ADDON_SLUG)

    @api_error("Failed to update the ZHAWS add-on")
    async def async_update_addon(self) -> None:
        """Update the ZHAWS add-on if needed."""
        addon_info = await self.async_get_addon_info()

        if addon_info.version is None:
            raise AddonError("ZHAWS add-on is not installed")
    ...
    def async_schedule_update_addon(self, catch_error: bool = False) -> asyncio.Task:
        """Schedule a task that updates and sets up the ZHAWS add-on.

        Only schedule a new update task if the there's no running task.
        """
        if not self._update_task or self._update_task.done():
            LOGGER.info("Trying to update the ZHAWS add-on")
    ...
    @api_error("Failed to stop the ZHAWS add-on")
    async def async_stop_addon(self) -> None:
        """Stop the ZHAWS add-on."""
    ...
    async def async_configure_addon(
        ...
        """Configure and start ZHAWS add-on."""
        addon_info = await self.async_get_addon_info()
    ...
    def async_schedule_setup_addon(
        ...
        """Schedule a task that configures and starts the ZHAWS add-on.
    ...
    @api_error("Failed to create a backup of the ZHAWS add-on.")
    async def async_create_backup(self) -> None:
        """Create a partial backup of the ZHAWS add-on."""
        addon_info = await self.async_get_addon_info()
...
class AddonError(HomeAssistantError):
    """Represent an error with ZHAWS add-on."""
